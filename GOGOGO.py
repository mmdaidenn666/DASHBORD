...
# ========== Template اصلی با استایل نئونی و موبایل‌محور ==========
BASE_TEMPLATE = '''
<!DOCTYPE html>
...
<body>
    ...
    <div class="bottom-nav">
        ...
        <a href="{{ url_for('chat_list') }}" class="nav-item {% if request.endpoint in ['chat_list', 'chat_page'] %}active{% endif %}">
            <i class="fas fa-comment"></i>
            <span>چت</span>
        </a>
        <a href="{{ url_for('view_notifications') }}" class="nav-item {% if request.endpoint == 'view_notifications' %}active{% endif %}" style="position: relative;">
...
REGISTER_TEMPLATE = '''
{% extends "base.html" %}
{% block title %}ثبت‌نام | GOYIMIX{% endblock %}
{% block content %}
    ...
    <form method="POST" style="background: rgba(255,255,255,0.05); padding: 30px; border-radius: 24px; backdrop-filter: blur(10px);">
        ...
        <select name="age" required style="text-align: center;">
            ...
            {% for i in range(12, 81) %}
                <option value="{{ i }}" {% if request.form.age == i|string %}selected{% endif %}>{{ i }} سال</option>
            {% endfor %}
        </select>
        
        <select name="gender" required style="text-align: center;">
            <option value="" disabled selected>جنسیت خود را انتخاب کنید</option>
            <option value="پسر" {% if request.form.gender == 'پسر' %}selected{% endif %}>پسر</option>
            <option value="دختر" {% if request.form.gender == 'دختر' %}selected{% endif %}>دختر</option>
            <option value="دیگر" {% if request.form.gender == 'دیگر' %}selected{% endif %}>دیگر</option>
        </select>
...
LOGIN_TEMPLATE = '''
{% extends "base.html" %}
{% block title %}ورود | GOYIMIX{% endblock %}
{% block content %}
...
DASHBOARD_TEMPLATE = '''
{% extends "base.html" %}
{% block title %}داشبورد | GOYIMIX{% endblock %}
{% block content %}
<div style="text-align: center; padding: 40px 20px;">
...
PROFILE_TEMPLATE = '''
{% extends "base.html" %}
{% block title %}پروفایل من | GOYIMIX{% endblock %}
{% block content %}
<div style="padding: 20px;">
    ...
    <div class="card">
        <div style="position: relative; padding: 15px 15px 15px 45px; border-bottom: 1px solid rgba(255,255,255,0.1);">
            <strong>نام:</strong> {{ user.name or "—" }}
    ...
    <div class="card" style="margin-top: 30px;">
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; padding: 15px;">
            ...
            <label class="toggle-switch">
                <input type="checkbox" onchange="toggleShowInHome(this)" {% if user.show_in_home %}checked{% endif %}>
...
EDIT_PROFILE_FIELD_TEMPLATE = '''
{% extends "base.html" %}
{% block title %}ویرایش {{ field|title }} | GOYIMIX{% endblock %}
{% block content %}
<div style="padding: 20px; max-width: 500px; margin: 0 auto;">
    <h1 style="text-align: center; margin-bottom: 30px;">ویرایش {{ field|title }}</h1>
    ...
    <div class="card">
        <form method="POST">
            ...
            {% elif field == 'age' %}
                <select name="new_value" required style="text-align: center;">
                    <option value="">سن جدید را انتخاب کنید</option>
                    {% for i in range(12, 81) %}
                        <option value="{{ i }}" {% if i == user.age %}selected{% endif %}>{{ i }} سال</option>
                    {% endfor %}
                </select>
            {% elif field == 'gender' %}
                <select name="new_value" required style="text-align: center;">
                    <option value="">جنسیت جدید را انتخاب کنید</option>
            ...
            {% elif field == 'city' %}
                <select name="new_value" required style="text-align: center;">
                    <option value="">شهر جدید را انتخاب کنید</option>
                    {% for city in cities %}
                        <option value="{{ city }}" {% if city == user.city %}selected{% endif %}>{{ city }}</option>
                    {% endfor %}
                </select>
            {% else %}
                <input type="text" name="new_value" placeholder="{{ field|title }} جدید" value="{{ user[field] if field in user else '' }}" required>
            {% endif %}
...
# ========== خانه (نمایش پروفایل‌ها با فیلتر) ==========
HOME_TEMPLATE = '''
{% extends "base.html" %}
{% block title %}خانه | GOYIMIX{% endblock %}
{% block content %}
<div style="padding: 20px;">
    ...
    <div style="text-align: center; margin-bottom: 30px; display: flex; flex-wrap: wrap; justify-content: center; gap: 10px;">
        <button class="filter-btn {% if 'same_age' in request.args %}active{% endif %}" data-filter="same_age">هم‌سن</button>
        <button class="filter-btn {% if 'same_gender' in request.args %}active{% endif %}" data-filter="same_gender">هم‌جنسیت</button>
        <button class="filter-btn {% if 'opposite_gender' in request.args %}active{% endif %}" data-filter="opposite_gender">ناهم‌جنسیت</button>
        <button class="filter-btn {% if 'same_city' in request.args %}active{% endif %}" data-filter="same_city">هم‌شهر</button>
    </div>
    ...
    {% if filtered_users|length > 0 %}
        ...
        <div class="card" style="display: flex; align-items: center; gap: 20px; padding: 20px;">
            ...
            <div style="flex: 1;">
                <h3 style="margin-bottom: 5px;">{{ user.name or user.username }}</h3>
                <p style="color: var(--light-gray); margin-bottom: 5px;">@{{ user.username }} • {{ user.age }} سال • {{ user.gender }}</p>
                {% if user.bio %}
                    <p style="margin-bottom: 5px;">{{ user.bio }}</p>
                {% endif %}
            ...
            <div style="display: flex; flex-direction: column; gap: 10px;">
                <form method="POST" action="{{ url_for('toggle_like', username=user.username) }}" style="margin: 0;">
                    <button type="submit" class="btn btn-small" style="padding: 10px 15px; background: {% if (session.username, user.username) in likes %}#ff3b30{% else %}linear-gradient(135deg, var(--primary), var(--secondary)){% endif %};">
                        {% if (session.username, user.username) in likes %}
...
{% extends "base.html" %}
{% block title %}اعلان‌ها | GOYIMIX{% endblock %}
{% block content %}
<div style="padding: 20px;">
        ...
        <div class="card" style="margin-bottom: 20px; padding: 20px; position: relative;">
            <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 15px;">
                <div class="avatar" style="width: 50px; height: 50px; font-size: 24px;">
                    {% if notif.from_user and notif.from_user in users %}
                        {{ users[notif.from_user].avatar }}
                    {% else %}
                        🔔
                    {% endif %}
                </div>
...
SEARCH_TEMPLATE = '''
{% extends "base.html" %}
{% block title %}جستجو | GOYIMIX{% endblock %}
{% block content %}
<div style="padding: 20px;">
...
<div style="padding: 20px;">
    ...
    {% if user_chat_list|length > 0 %}
        ...
        <div class="card" style="display: flex; align-items: center; gap: 20px; padding: 20px; position: relative;">
            ...
            <div style="flex: 1;">
                <h3 style="margin-bottom: 5px;">{{ chat.other_user.name or chat.other_user.username }}</h3>
...
CHAT_PAGE_TEMPLATE = '''
{% extends "base.html" %}
{% block title %}چت با {{ target_user.name or target_user.username }} | GOYIMIX{% endblock %}
{% block content %}
<div style="display: flex; flex-direction: column; min-height: 100vh;">
    ...
    <div style="flex: 1; overflow-y: auto; padding: 20px;">
        {% if chat_messages|length > 0 %}
            ...
            <div style="display: flex; {% if msg.from == session.username %}justify-content: flex-end;{% endif %}">
                <div class="message-container">
                    <div class="message {% if msg.from == session.username %}sent{% else %}received{% endif %}">
                        {{ msg.text }}
...
def profile():
    ...
    if not user:
        ...
        return redirect(url_for('login'))
    return render_template_string(PROFILE_TEMPLATE, user=user, likes=likes)
    ...
    if request.method == 'POST':
        if field == 'password':
            ...
            if not check_password_hash(user['password'], current_password):
                ...
                return render_template_string(EDIT_PROFILE_FIELD_TEMPLATE, field=field, user=user, cities=CITIES)
...
def home():
    ...
    return render_template_string(HOME_TEMPLATE, filtered_users=filtered_users, likes=likes)
...
def view_notifications():
    ...
    for notif in user_notifications:
        notif['read'] = True
    return render_template_string(NOTIFICATIONS_TEMPLATE, user_notifications=user_notifications, users=users)
        ...
        if query.startswith('@'):
            ...
            for user in users.values():
                if query.lower() in (user.get('name', '').lower() or user['username'].lower()):
                    results.append(user)
    return render_template_string(SEARCH_TEMPLATE, results=results, query=query, likes=likes)
...
def chat_list():
    ...
    return render_template_string(CHAT_LIST_TEMPLATE, user_chat_list=user_chat_list)
...
def chat_page(chat_id):
    ...
    return render_template_string(CHAT_PAGE_TEMPLATE, target_user=target_user, chat_id=chat_id, chat_messages=chats[chat_id]['messages'])
