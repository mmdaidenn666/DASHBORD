...
@app.route('/')
def home():
    # هدایت خودکار اگر لاگین شده باشد
...
<head>
    ...
    <style>
        ...
        .btn-primary:hover { background: linear-gradient(90deg, #FF3CAC, #00FF9C); box-shadow: 0 0 20px #FF3CAC, 0 0 40px #00FF9C; transform: translateY(-3px); }
...
<head>
    ...
    <style>
        ...
        .btn-primary:hover { background: linear-gradient(90deg, #00FF9C, #FF3CAC); box-shadow: 0 0 20px #00FF9C, 0 0 40px #FF3CAC; transform: translateY(-3px); }
        .alert { padding: 12px; background: #331111; border: 1px solid #EF4444; border-radius: 8px; color: #FF6B6B; text-align: center; margin-bottom: 20px; font-size: 16px; }
...
<body>
    <div class="form-container">
        ...
        <form method="POST" id="teacherForm">
            ...
            <div class="sub-group">
                ...
                <div class="form-group">
                    <label>پایه {{ i }}</label>
                    <select name="grade_{{ i }}" id="grade{{ i }}" {{ 'disabled' if request.form.grade_count == '0' or (request.form.grade_count and i > int(request.form.grade_count)) }}>
                        <option value="">انتخاب کنید...</option>
            ...
            <div class="sub-group">
                ...
                <div class="form-group">
                    <label>رشته {{ i }}</label>
                    <select name="field_{{ i }}" id="field{{ i }}" {{ 'disabled' if request.form.field_count == '0' or (request.form.field_count and i > int(request.form.field_count)) }}>
                        <option value="">انتخاب کنید...</option>
            ...
            <div class="sub-group">
                ...
                <div class="form-group">
                    <label>درس {{ i }}</label>
                    <input type="text" name="subject_{{ i }}" id="subject{{ i }}" value="{{ request.form.get('subject_' + i|string) or '' }}" {{ 'disabled' if request.form.subject_count == '0' or (request.form.subject_count and i > int(request.form.subject_count)) }}>
                </div>
                {% endfor %}
            </div>
    ...
    <script>
        // فعال/غیرفعال کردن فیلدها بر اساس انتخاب تعداد
    ...
    <script>
        ...
        function editField(field, btn) {
            ...
            if (field === 'role') {
                ...
                input.type = 'text'; input.value = currentValue; input.style.width = '100%'; input.style.padding = '10px'; input.style.background = '#222'; input.style.color = 'white'; input.style.borderRadius = '10px'; input.style.border = '1px solid #444'; input.style.direction = 'rtl'; input.style.fontSize = '18px';
...
<body>
    ...
    <script>
        ...
        function editField(field, btn) {
            ...
            // برای آرایه‌ها
            if (field.endsWith('s')) {
                currentValue = currentValue === 'ندارد' ? [] : currentValue.split(', ').map(s => s.trim());
            }
            ...
            if (field === 'grade_count' || field === 'field_count' || field === 'subject_count') {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    opts.forEach(optText => {
                        ...
                        if (currentValue[i] === optText) opt.selected = true;
                        select.appendChild(opt);
                    });
...
def update_teacher_profile():
    ...
    if field in allowed_fields:
        if field in ['grades', 'fields', 'subjects']:
            ...
            session['teacher'][field] = value.strip() if isinstance(value, str) else value
        return jsonify(success=True)
    return jsonify(success=False), 400
