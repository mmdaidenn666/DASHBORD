        ...
        if role == "admin":
            ...
            for i, t in enumerate(data_db.get("teachers", [])):
                if t.get("id") == user["id"]:
                    ...
                    break
        save_data(data_db)
        session[f"{role}_id"] = user["id"] # Update session if needed
...
def field_page(grade, field):
    ...
    <body>
        <button class="search-btn" onclick="window.location.href='/search/{grade}/{field}'">🔍</button>
        <div class="container">
            <div class="header">
                <h1>{field_name}</h1>
                <div class="counter">
                    <i>👤</i>
                    <span id="student-count">{len(students)}</span>
                </div>
            </div>
            <div class="students-grid" id="students-list">
                {''.join([
                    ...
                    <div class="student-card" onclick="viewStudent({s['id']})">
                        ...
                        <div class="actions">
                            <button class="card-btn edit-card-btn" onclick="editStudent(event, {s['id']}); event.stopPropagation();">✎</button>
                            <button class="card-btn delete-card-btn" onclick="deleteStudent(event, {s['id']}); event.stopPropagation();">🗑️</button>
                        </div>
                    </div>
                    '''
                    for s in students
...
@app.route("/search/<int:grade>/<string:field>")
def search_page(grade, field):
    user, role = get_current_user()
    if not user or role != "admin":
        return redirect(url_for("index"))
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>جستجو - {field_name}</h1>
            </div>
            <div class="search-form">
                <div class="form-group">
                    <label>جستجو بر اساس: نام، نام خانوادگی، کد ملی، یا همه</label>
                    <input type="text" id="search-input" placeholder="مثلاً: محمدرضا محمدی یا 1234567890">
                </div>
                <button class="submit-btn" onclick="performSearch()">جستجو</button>
            </div>
        ...
        <script>
            ...
            function performSearch() {{
                ...
                fetch('/api/search', {{
                    method: 'POST',
                    headers: {{ 'Content-Type': 'application/json' }},
                    body: JSON.stringify({{ query: query, grade: currentGrade, field: currentField }})
                }})
                .then(response => response.json())
                .then(data => {{
                    ...
                    resultsDiv.innerHTML = data.results.map(s => `
                        <div class="student-card" onclick="viewStudent(${s.id})">
                            <div class="student-info">
                                <div class="student-name">${s.name} ${s.family}</div>
                                <div class="student-national-id">${s.national_id}</div>
                                <div class="student-grade-field">پایه ${s.grade} - {field_name}</div>
                            </div>
                        </div>
                    `).join('');
                }});
...
def api_search():
    ...
    grade_filter = request.json.get("grade", None)
    field_filter = request.json.get("field", "")

    results = []
    for grade, fields in data.get("students", {}).items():
        if grade_filter and str(grade_filter) != grade:
            continue
        for field, students in fields.items():
            if field_filter and field_filter != field:
                continue
            for s in students:
                if (query in s["name"].lower() or
                    query in s["family"].lower() or
                    query in s["national_id"] or
                    query in f"{s['name']} {s['family']}".lower()):
                    s_copy = s.copy()
                    s_copy["grade"] = int(grade)
