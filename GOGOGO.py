...
blocked_users = {}  # {username: [blocked_user1, blocked_user2, ...]}
...
def add_notification(target_user, msg, type="info", from_user=None):
    if target_user in blocked_users and from_user in blocked_users[target_user]:
        return
    notif = {
        "id": str(uuid.uuid4()),
        "msg": msg,
...
def is_blocked(current_user, target_user):
    return current_user in blocked_users and target_user in blocked_users[current_user]
        ...
        {% with messages = get_flashed_messages(with_categories=true) %}
            {% if messages %}
                {% for category, message in messages %}
                    <div style="background: {% if category == 'error' %}#ff3b30{% else %}#00F5D4{% endif %}; 
                               color: {% if category == 'error' %}white{% else %}black{% endif %}; 
                               padding: 15px; 
                               text-align: center; 
                               margin: 10px 0; 
                               border-radius: 12px; 
...
def home():
    ...
    for user in filtered_users:
        ...
        heart_color = "#ff3b30" if is_liked else "linear-gradient(135deg, var(--primary), var(--secondary))"
        ...
        <div class="card" style="display: flex; align-items: center; gap: 20px; padding: 20px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 10px;">
                <form method="POST" action="{{{{ url_for('toggle_like', username='{user['username']}') }}}}" style="margin: 0;">
                    <button type="submit" class="btn btn-small" style="padding: 10px 15px; background: {heart_color};">
...
def block_user(username):
    ...
    if username not in blocked_users[current_user]:
        blocked_users[current_user].append(username)
...
def search():
    ...
    if query:
        if query.startswith('@'):
            username = query[1:]
            if username in users and not is_blocked(session['username'], username):
                results.append(users[username])
        else:
            for user in users.values():
                if not is_blocked(session['username'], user['username']) and (query.lower() in (user.get('name', '').lower() or user['username'].lower())):
                    results.append(user)
    ...
    <div style="padding: 20px;">
        ...
        <form method="GET" style="margin-bottom: 30px;">
            <div style="display: flex; gap: 10px;">
                <input type="text" name="q" placeholder="جستجو بر اساس نام یا @نام کاربری" value="{{ request.args.q or '' }}" style="flex: 1;">
...
def chat_with_user(username):
    ...
    for user in [current_user, username]:
        if user not in user_chats:
            user_chats[user] = []
        if chat_id not in user_chats[user]:
            user_chats[user].append(chat_id)
    ...
    <div style="padding: 20px;">
        ...
        <div class="card">
            <div style="position: relative; padding: 15px 15px 15px 45px; border-bottom: 1px solid rgba(255,255,255,0.1);">
                <strong>نام:</strong> ''' + (user.get('name', '—') or '—') + '''
            ...
            <div style="position: relative; padding: 15px 15px 15px 45px; border-bottom: 1px solid rgba(255,255,255,0.1);">
                <strong>سن:</strong> ''' + str(user['age']) + ''' سال
            ...
            <div style="position: relative; padding: 15px 15px 15px 45px; border-bottom: 1px solid rgba(255,255,255,0.1);">
                <strong>بیو:</strong> ''' + (user.get('bio', '—') or '—') + '''
        ...
        <div class="card" style="margin-top: 30px;">
            ...
            <div style="display: flex; align-items: center; justify-content: space-between; padding: 15px;">
                ...
                <label class="toggle-switch">
                    <input type="checkbox" onchange="toggleShowInHome(this)" {% if ''' + ('true' if user.get('show_in_home', True) else 'false') + ''' %}checked{% endif %}>
...
def edit_profile_field(field):
    ...
    if request.method == 'POST':
        if field == 'password':
            ...
            if not check_password_hash(user['password'], current_password):
                ...
                <div style="padding: 20px; max-width: 500px; margin: 0 auto;">
                    <h1 style="text-align: center; margin-bottom: 30px;">ویرایش رمز عبور</h1>
        ...
        elif field in ['age', 'gender', 'city', 'name', 'bio', 'username']:
            if field == 'username':
                new_username = request.form['new_value'].strip()
                if not new_username:
                    flash('نام کاربری نمی‌تواند خالی باشد', 'error')
                    return render_template_string('''
                    {% extends "base.html" %}
                    {% block content %}
                    <div style="padding: 20px; max-width: 500px; margin: 0 auto;">
                        <h1 style="text-align: center; margin-bottom: 30px;">ویرایش نام کاربری</h1>
                        
                        <div class="card">
                            <form method="POST">
                                <input type="text" name="new_value" placeholder="نام کاربری جدید" value="{{ request.form.new_value or '' }}" required>
                ...
                users[new_username] = users.pop(user['username'])
    ...
    if field == 'password':
        ...
        field_input = f'<input type="text" name="new_value" placeholder="{field_label} جدید" value="{user.get(field, "")}" required>'
    
    return render_template_string('''
    {% extends "base.html" %}
    {% block content %}
    <div style="padding: 20px; max-width: 500px; margin: 0 auto;">
        <h1 style="text-align: center; margin-bottom: 30px;">ویرایش ''' + field_label + '''</h1>
