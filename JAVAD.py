...
def home():
    if session.get('admin'):
        init_students()  # مطمئن شویم students وجود دارد
...
def admin_login():
    ...
    if request.method == 'POST':
        ...
        if not all([name, family, role, password]):
            ...
            init_students()  # مقداردهی اولیه دانش‌آموزان
            return redirect(url_for('admin_dashboard'))
...
def select_field(grade):
    ...
    count = len(session['students'][grade]['ریاضی']) + len(session['students'][grade]['تجربی']) + len(session['students'][grade]['انسانی'])
    return render_template_string('''
<!DOCTYPE html>
<html lang="fa" dir="rtl">
<head>
