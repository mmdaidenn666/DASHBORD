...
<body>
    ...
    <script>
        function editField(field, btn) {
            // ذخیره مرجع دکمه برای بازیابی بعداً
            btn.dataset.original = 'true';
            ...
            controls.innerHTML = `<button class="btn-ghost" onclick="cancelEdit('${field}', this)">انصراف</button><button class="btn-primary" onclick="saveEdit('${field}', this)">تایید</button>`;
        ...
        function cancelEdit(field, btn) {
            // بازیابی مقدار اصلی
            const valueEl = document.getElementById(field + 'Value');
            const originalValue = valueEl.previousElementSibling.innerText;
            valueEl.parentNode.innerHTML = `<div class="field-value" id="${field}Value">${originalValue}</div>`;
            // بازیابی دکمه مداد
            const editBtn = document.createElement('button');
            editBtn.className = 'edit-btn';
            editBtn.innerHTML = '<i class="fas fa-pencil-alt"></i>';
            editBtn.onclick = () => editField(field, editBtn);
            valueEl.parentNode.appendChild(editBtn);
        }
        ...
        function saveEdit(field, btn) {
            ...
            fetch('/admin/profile', {
                ...
                if (data.success) {
                    // به‌روزرسانی UI
                    const valueEl = document.getElementById(field + 'Value');
                    valueEl.parentNode.innerHTML = `<div class="field-value" id="${field}Value">${newValue}</div>`;
                    // بازیابی دکمه مداد
                    const editBtn = document.createElement('button');
                    editBtn.className = 'edit-btn';
                    editBtn.innerHTML = '<i class="fas fa-pencil-alt"></i>';
...
<body>
    ...
    <script>
        ...
        function editStudent(e, index) {
            ...
            document.getElementById('addStudentModal').dataset.editGrade = grade;
            document.getElementById('addStudentModal').dataset.editField = field;
        }
    ...
    <script>
        function editField(field, btn) {
            ...
            if (field === 'grade_count' || field === 'field_count' || field === 'subject_count') {
                ...
                const currentArray = currentValue === 'ندارد' ? [] : currentValue.split(', ').map(s => s.trim());
                for (let i = 0; i < count; i++) {
                    ...
                    opts.forEach(optText => {
                        ...
                        if (i < currentArray.length && optText === currentArray[i]) {
                            opt.selected = true;
                        }
                        select.appendChild(opt);
                    });
                ...
                for (let i = 0; i < count; i++) {
                    const input = document.createElement('input');
                    input.type = 'text';
                    input.value = i < currentArray.length ? currentArray[i] : '';
...
def admin_logout():
    ...
    # session.pop('students', None)  # اگر می‌خواهید داده‌ها پاک شوند
