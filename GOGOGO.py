            ...
            {{% if messages %}}
                {{% for category, message in messages %}}
                    <div style="background: {{ '{{' }} category == 'error' and '#ff3b30' or '#00F5D4' {{ '}}' }}; 
                               color: {{ '{{' }} category == 'error' and 'white' or 'black' {{ '}}' }}; 
                               padding: 15px; 
                               text-align: center; 
                               margin: 10px 0; 
                               border-radius: 12px; 
...
def home():
    ...
    <div style="padding: 20px;">
        ...
        <div style="text-align: center; margin-bottom: 30px; display: flex; flex-wrap: wrap; justify-content: center; gap: 10px;">
            <button class="filter-btn {'active' if request.args.get('same_age') == '1' else ''}" data-filter="same_age">هم‌سن</button>
            <button class="filter-btn {'active' if request.args.get('same_gender') == '1' else ''}" data-filter="same_gender">هم‌جنسیت</button>
            <button class="filter-btn {'active' if request.args.get('opposite_gender') == '1' else ''}" data-filter="opposite_gender">ناهم‌جنسیت</button>
            <button class="filter-btn {'active' if request.args.get('same_city') == '1' else ''}" data-filter="same_city">هم‌شهر</button>
        </div>
...
def chat_page(chat_id):
    ...
    <div style="display: flex; flex-direction: column; min-height: 100vh;">
        <div class="chat-header">
            ...
            <form method="POST" action="{{{{ url_for('delete_chat', chat_id='{chat_id}') }}}}">
...
def profile():
    ...
    '''
    return make_page("پروفایل من", content)
...
def edit_profile_field(field):
    ...
    if request.method == 'POST':
        if field == 'password':
            ...
            if not check_password_hash(user['password'], current_password):
                ...
                '''
                return make_page("ویرایش رمز عبور", content)
            ...
            if field == 'username':
                ...
                if not new_username:
                    ...
                    '''
                    return make_page("ویرایش نام کاربری", content)
    ...
    '''
    return make_page(f"ویرایش {field_label}", content)
