    ...
    <style>
        ...
        * {
        ...
        .form-container {
            ...
            margin-top: 60px; /* جابجایی کادر پایین دکمه برگشت */
        }
        .back-btn {
            position: absolute;
            top: 20px;
...
def announcements():
    ...
    # برای این نسخه، اعلان‌های تستی حذف شده‌اند.
    announcements_list = [] # تهی کردن لیست
    ...
    <body>
        <div class="container">
            ...
            <div class="announcements-list">
                ...
                <p style="text-align: center; color: #ccc;">هیچ اعلانی وجود ندارد.</p>
                {% endif %}
            </div>
    ...
    '''
    return render_template_string(html, announcements_list=announcements_list)
        ...
        <style>
            ...
            * {{
            ...
            @media (max-width: 768px) {{
                ...
                .profile-label, .profile-value, .edit-field {{ width: 100%; }}
            }}
        </style>
    </head>
    <body>
    ...
    <body>
                    ...
                    <div class="student-card" onclick="showStudentDetails({s['id']})">
                        <div class="student-card-header">
                            ...
                            <div class="student-card-actions">
                                <button class="edit-card-btn" onclick="editStudent({s['id']}); event.stopPropagation();">✎</button>
                                <button class="delete-card-btn" onclick="deleteStudent({s['id']}); event.stopPropagation();">🗑️</button>
                            </div>
                        </div>
        ...
        <script>
            ...
            function editStudent(id) {{
                // Navigate to the edit page (same as add page but pre-filled)
...
def add_student_page(grade, field):
    ...
    if request.method == "POST":
        ...
        # Check for duplicate national_id if adding new or changing national_id on edit
...
def view_student(student_id):
    ...
    for grade_data in data.get("students", {}).values():
        for field_data in grade_data.values():
            for s in field_data: # --- اصلاح این خط ---
                if s.get("id") == student_id:
                    student = s
                    break
            if student:
...
@app.route("/api/student-count/<int:grade>")
def api_student_count(grade):
    data = load_data()
    count = 0
    for field, students in data.get("students", {}).get(str(grade), {}).items():
        count += len(students)
