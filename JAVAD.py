...
<head>
    ...
    <style>
        ...
        .field-value { flex: 1; padding: 12px; background: #222; border-radius: 12px; direction: rtl; font-size: 18px; min-width: 150px; }
        .edit-btn { background: #222; color: #00FF9C; border: 1px solid #00FF9C; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; box-shadow: 0 0 8px rgba(0, 255, 156, 0.5); font-size: 16px; transition: all 0.3s; }
        .edit-btn:hover { transform: scale(1.1); }
        .edit-controls { display: flex; gap: 12px; margin-top: 12px; width: 100%; animation: fadeIn 0.3s ease; }
...
<body>
    ...
    <div class="container">
        ...
        <div class="profile-field">
            <div class="field-label">مرتبه:</div>
            <div class="field-value">استاد</div>
...
def parent_login():
    ...
    if request.method == 'POST':
        ...
        if not all([name, family, grade, field, child1, password]):
            ...
            session['parent'] = {
                ...
                'field': field,
                'child1': child1,
                'child2': child2 if child2 else None
            }
            session.modified = True
            return redirect(url_for('parent_dashboard'))
    ...
    init_students()
    students_data = {}
    for grade in ['دهم', 'یازدهم', 'دوازدهم']:
        for field in ['ریاضی', 'تجربی', 'انسانی']:
            students = session['students'][grade][field]
            if students:
                students_data[f'{grade}_{field}'] = [
                    {'name': s['name'], 'family': s['family'], 'national_id': s['national_id'][-4:]} 
                    for s in students
                ]
...
<head>
    ...
    <style>
        body { background: #0D0D0D; color: white; font-family: 'Vazir', sans-serif; padding: 15px; display: flex; justify-content: center; align-items: center; min-height: 100vh; }
        .form-container { background: #1A1A1A; padding: 25px; border-radius: 20px; width: 100%; max-width: 600px; box-shadow: 0 0 20px rgba(157, 78, 221, 0.3); border: 1px solid #333; }
...
<body>
    ...
    <script>
        ...
        function updateStudents() {
            ...
            if (gradeSelect.value && fieldSelect.value) {
                const key = gradeSelect.value + '_' + fieldSelect.value;
...
<head>
    ...
    <style>
        ...
        input, select { width: 100%; padding: 12px; background: #222; border: 1px solid #444; border-radius: 10px; color: white; font-size: 16px; direction: rtl; }
        .btn-primary { width: 100%; padding: 14px; background: linear-gradient(90deg, #00FF9C, #FF3CAC); color: #0D0D0D; border: none; border-radius: 14px; font-weight: bold; font-size: 18px; cursor: pointer; box-shadow: 0 0 12px #00FF9C, 0 0 24px #FF3CAC; }
        .alert { padding: 12px; background: #331111; border: 1px solid #EF4444; border-radius: 8px; color: #FF6B6B; text-align: center; margin-bottom: 20px; font-size: 16px; }
    ...
    <div class="form-container">
        ...
        <form method="POST" id="studentForm">
            ...
            <div class="form-group">
                ...
                <input type="text" name="national_id" value="{{ request.form.national_id or '' }}" required pattern="[0-9]{10}" title="لطفاً 10 رقم عددی وارد کنید">
            </div>
...
def student_profile():
    ...
    if request.method == 'POST':
        ...
        field = data.get('field')
        value = data.get('value')
        if field in ['name', 'family', 'grade', 'field', 'national_id', 'password'] and value:
            session['student'][field] = value
            session.modified = True
            return jsonify(success=True)
        return jsonify(success=False, error="مقدار نامعتبر"), 400
...
<head>
    ...
    <style>
        ...
        .edit-btn:hover { transform: scale(1.1); }
        .edit-controls { display: flex; gap: 12px; margin-top: 12px; width: 100%; animation: fadeIn 0.3s ease; }
        .btn-ghost { flex: 1; padding: 10px; background: transparent; border: 1px solid #00FF9C; color: #00FF9C; border-radius: 12px; cursor: pointer; font-size: 16px; transition: all 0.3s; }
        .btn-ghost:hover { background: rgba(0, 255, 156, 0.1); }
        .btn-primary { flex: 1; padding: 10px; background: linear-gradient(90deg, #00FF9C, #FF3CAC); color: #0D0D0D; border: none; border-radius: 12px; font-weight: bold; font-size: 16px; cursor: pointer; box-shadow: 0 0 10px #00FF9C, 0 0 20px #FF3CAC; transition: all 0.3s; }
        .btn-primary:hover { transform: translateY(-2px); }
        .btn-logout { width: 100%; padding: 16px; background: linear-gradient(90deg, #EF4444, #FF3CAC); color: white; border: none; border-radius: 16px; font-size: 18px; margin-top: 30px; cursor: pointer; box-shadow: 0 0 12px #EF4444, 0 0 24px #FF3CAC; transition: all 0.3s; }
        .btn-logout:hover { transform: translateY(-2px); }
        .modal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); justify-content: center; align-items: center; z-index: 2000; animation: fadeIn 0.3s ease; }
...
<body>
    ...
    <div class="container">
        ...
        <div class="profile-field">
            ...
            <button class="edit-btn" onclick="editField('national_id', this)"><i class="fas fa-pencil-alt"></i></button>
        </div>
        
        <div class="profile-field">
            <div class="field-label">رمز عبور:</div>
            <div class="field-value" id="passwordValue">{{ user.password }}</div>
            <button class="edit-btn" onclick="editField('password', this)"><i class="fas fa-pencil-alt"></i></button>
        </div>
