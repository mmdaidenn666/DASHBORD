...
# داده‌های اعلانات
announcements = []
# داده‌های اعلانات معلمان (هر معلم اعلانات خودش را دارد)
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .search-announcements-btn:hover {
            transform: scale(1.1);
        }
        /* دکمه غیرفعال */
        .btn-disabled {
                ...
                <div class="form-container">
                    <form method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select id="teacher_field" name="teacher_field" required>
                                <option value="">انتخاب کنید</option>
                                <option value="math" {{ 'selected' if request.form.teacher_field == 'math' }}>ریاضی</option>
                                <option value="science" {{ 'selected' if request.form.teacher_field == 'science' }}>تجربی</option>
                                <option value="humanities" {{ 'selected' if request.form.teacher_field == 'humanities' }}>انسانی</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="teacher_subject">درس:</label>
                            <input type="text" id="teacher_subject" name="teacher_subject" value="{{ request.form.teacher_subject or '' }}" required>
                        </div>
                        <div class="form-group">
        ...
        {% else %}
            {% if page == 'profile' %}
                ...
                <div class="profile-container">
                    ...
                    <div class="profile-item">
                        <div>
                            <div class="profile-label">مرتبه:</div>
                            <div class="profile-value">{{ session.role }}</div>
                        </div>
                    ...
                    <div class="profile-item">
                        <div>
                            ...
                            <div class="profile-value" id="grade-value">{{ session.grade_name }}</div>
                        </div>
                        <button class="edit-btn" onclick="editField('grade', '{{ session.grade }}')">✎</button>
                    </div>
                    <div class="profile-item">
                        <div>
                            <div class="profile-label">رشته:</div>
                            <div class="profile-value" id="field-value">{{ session.field_name }}</div>
                        </div>
                        <button class="edit-btn" onclick="editField('field', '{{ session.field }}')">✎</button>
                    </div>
                    <div class="profile-item">
                        <div>
                            <div class="profile-label">درس:</div>
                            <div class="profile-value" id="subject-value">{{ session.subject }}</div>
                        </div>
                        <button class="edit-btn" onclick="editField('subject', '{{ session.subject }}')">✎</button>
                    </div>
                    {% endif %}
                    <div class="profile-item">
                        <div>
            ...
            {% elif page == 'announcements' %}
                ...
                <div class="announcement-header">
                    <button class="search-announcements-btn" onclick="window.location.href='/{{ 'admin' if session.role == 'مدیر' else 'teacher' }}/announcements/search'">🔍</button>
                </div>
                {% if session.role == 'مدیر' and announcements %}
                ...
                {% elif session.role != 'مدیر' and teacher_announcements.get(session.teacher_key, []) %}
                    <div class="announcements-grid">
                        {% for announcement in teacher_announcements[session.teacher_key] %}
                        <div class="announcement-card">
                            <div class="announcement-header">
                                <div class="announcement-title">{{ announcement.title }}</div>
                                <!-- معلمان نمی‌توانند اعلانات خودشان را حذف کنند -->
                            </div>
                            <div class="announcement-content">{{ announcement.content|safe }}</div>
                            <div class="announcement-meta">
                                <div>تاریخ: {{ announcement.date }}</div>
            ...
            {% elif page == 'attendance' %}
                ...
                <div class="grid-buttons">
                    {% set teacher_grade = session.grade %}
                    <button class="btn btn-grade {% if teacher_grade != '10' %}btn-disabled{% endif %}" 
                            onclick="window.location.href='/teacher/attendance/10'" 
                            {% if teacher_grade != '10' %}disabled{% endif %}>پایه دهم</button>
                    <button class="btn btn-grade {% if teacher_grade != '11' %}btn-disabled{% endif %}" 
                            onclick="window.location.href='/teacher/attendance/11'" 
                            {% if teacher_grade != '11' %}disabled{% endif %}>پایه یازدهم</button>
                    <button class="btn btn-grade {% if teacher_grade != '12' %}btn-disabled{% endif %}" 
                            onclick="window.location.href='/teacher/attendance/12'" 
                            {% if teacher_grade != '12' %}disabled{% endif %}>پایه دوازدهم</button>
                </div>
            {% elif page == 'attendance_grade' %}
                ...
                <div class="grid-buttons">
                    {% set teacher_field = session.field %}
                    <button class="btn btn-field {% if teacher_field != 'math' %}btn-disabled{% endif %}" 
                            onclick="window.location.href='/teacher/attendance/{{ grade }}/math'" 
                            {% if teacher_field != 'math' %}disabled{% endif %}>رشته ریاضی</button>
                    <button class="btn btn-field {% if teacher_field != 'science' %}btn-disabled{% endif %}" 
                            onclick="window.location.href='/teacher/attendance/{{ grade }}/science'" 
                            {% if teacher_field != 'science' %}disabled{% endif %}>رشته تجربی</button>
                    <button class="btn btn-field {% if teacher_field != 'humanities' %}btn-disabled{% endif %}" 
                            onclick="window.location.href='/teacher/attendance/{{ grade }}/humanities'" 
                            {% if teacher_field != 'humanities' %}disabled{% endif %}>رشته انسانی</button>
                </div>
            ...
            if (field === 'role') {
                ...
                valueElement.innerHTML = `
                    <form class="inline-edit-form" method="POST" action="/edit_profile">
                        <input type="hidden" name="field" value="${field}">
                        <select name="value" class="inline-edit-select">
                            <option value="10" ${currentValue === '10' ? 'selected' : ''}>دهم</option>
                            <option value="11" ${currentValue === '11' ? 'selected' : ''}>یازدهم</option>
                            <option value="12" ${currentValue === '12' ? 'selected' : ''}>دوازدهم</option>
                        </select>
                        <div class="inline-edit-buttons">
                            <button type="submit" class="inline-edit-btn save-btn">✓ تایید</button>
                            <button type="button" class="inline-edit-btn cancel-btn" onclick="cancelEdit('${field}', '${currentValue}', this)">✗ انصراف</button>
                ...
                valueElement.innerHTML = `
                    <form class="inline-edit-form" method="POST" action="/edit_profile">
                        <input type="hidden" name="field" value="${field}">
                        <select name="value" class="inline-edit-select">
                            <option value="math" ${currentValue === 'math' ? 'selected' : ''}>ریاضی</option>
                            <option value="science" ${currentValue === 'science' ? 'selected' : ''}>تجربی</option>
                            <option value="humanities" ${currentValue === 'humanities' ? 'selected' : ''}>انسانی</option>
                        </select>
                        <div class="inline-edit-buttons">
                            <button type="submit" class="inline-edit-btn save-btn">✓ تایید</button>
                            <button type="button" class="inline-edit-btn cancel-btn" onclick="cancelEdit('${field}', '${currentValue}', this)">✗ انصراف</button>
...
def login_teacher():
    if request.method == 'POST':
        ...
        if password == 'Dabirjavad01':
            ...
            # تبدیل کد عددی پایه و رشته به نام نمایشی
            ...
            session['teacher_key'] = f"{name}_{lastname}_{grade}_{field}_{subject}"
...
def teacher_announcements():
    if not session.get('logged_in') or session.get('user_type') != 'teacher':
        return redirect('/')
    teacher_key = session.get('teacher_key')
    teacher_anns = teacher_announcements.get(teacher_key, [])
    return render_template_string(HTML_TEMPLATE, page='announcements', announcements=teacher_anns)

@app.route('/admin/announcements/search', methods=['GET', 'POST'])
def admin_search_announcements():
    ...
    return render_template_string(HTML_TEMPLATE, page='search_announcements')

@app.route('/teacher/announcements/search', methods=['GET', 'POST'])
def teacher_search_announcements():
    ...
    if request.method == 'POST':
        ...
        for announcement in teacher_anns:
            # اینجا باید منطق فیلتر کردن را پیاده کنید
            # فرض می‌کنیم که تاریخ در فیلد date به فرمت "1404/09/01" ذخیره شده است
            if announcement.get('date'):
                date_parts = announcement['date'].split('/')
...
def attendance_field(grade, field):
    ...
    # بررسی اینکه آیا پایه و رشته انتخاب شده با اطلاعات معلم مطابقت دارد
    if session.get('grade') != grade or session.get('field') != field:
...
def submit_attendance(grade, field):
    ...
    teacher_grade = session['grade_name']
    teacher_field = session['field_name']
    ...
    field_key = f"{field}_{grade}"
    for i, student in enumerate(students[field_key]):
        if str(i) in request.form.get('attendance', {}):
            status = request.form['attendance'][str(i)]
            if status == 'present':
                present_students.append(f"{student['name']} {student['lastname']}")
            elif status == 'absent':
                absent_students.append(f"{student['name']} {student['lastname']}")
    
    # ایجاد محتوای اعلان
    announcement_content = f"""
    <p><strong>نام معلم:</strong> {teacher_name} {teacher_lastname}</p>
    <p><strong>پایه:</strong> {teacher_grade}</p>
    <p><strong>رشته:</strong> {teacher_field}</p>
    ...
    <div style="display: flex; justify-content: space-between;">
        <div style="color: #ff4444;">
            ...
            <ul>
                {''.join([f'<li>{student}</li>' for student in absent_students]) if absent_students else '<li>هیچ</li>'}
            </ul>
        </div>
        <div style="color: #00ff00;">
            <p><strong>حاضرین:</strong></p>
            <ul>
                {''.join([f'<li>{student}</li>' for student in present_students]) if present_students else '<li>هیچ</li>'}
            </ul>
        </div>
    </div>
    """
    ...
    # اضافه کردن به اعلانات معلم
    if teacher_key not in teacher_announcements:
        teacher_announcements[teacher_key] = []
    teacher_announcements[teacher_key].append(teacher_announcement)
...
def edit_profile():
    ...
    if field in ['name', 'lastname', 'role']:
        ...
        session['grade'] = value
        grade_names = {'10': 'دهم', '11': 'یازدهم', '12': 'دوازدهم'}
        session['grade_name'] = grade_names.get(value, value)
    elif field == 'field':
        session['field'] = value
        field_names = {'math': 'ریاضی', 'science': 'تجربی', 'humanities': 'انسانی'}
        session['field_name'] = field_names.get(value, value)
    elif field == 'subject':
