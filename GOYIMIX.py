...
class Notification(db.Model):
    ...
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)
    type = db.Column(db.String(50), default='info')  # like, chat_request, message, welcome
...
class WelcomeNotification(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, nullable=False)  # کاربری که اعلان دریافت می‌کند
    new_user_id = db.Column(db.Integer, nullable=False)  # کاربر جدید
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)
...
def register():
    if request.method == 'POST':
        ...
        # ایجاد اعلان خوش‌آمدگویی برای کاربران مخالف جنسیت
        opposite_gender = 'دختر' if gender == 'پسر' else 'پسر'
        ...
        for matching_user in matching_users:
            welcome_notif = WelcomeNotification(
            ...
            notif = Notification(
                user_id=matching_user.id,
                message=f"کاربر جدید @{user.username} ({user.name or 'بدون نام'}) وارد سایت شد",
                type='welcome',
                sender_id=user.id
            )
...
def profile():
    ...
    return render_template_string(PROFILE_TEMPLATE, like_count=like_count, cities=CITIES)
...
def delete_account():
    ...
    WelcomeNotification.query.filter_by(user_id=current_user.id).delete()
    WelcomeNotification.query.filter_by(new_user_id=current_user.id).delete()
...
def notifications():
    # اعلان‌های عادی
    regular_notifs = Notification.query.filter_by(user_id=current_user.id).order_by(Notification.timestamp.desc()).all()
    
    # اعلان‌های خوش‌آمدگویی
    welcome_notifs = WelcomeNotification.query.filter_by(user_id=current_user.id).order_by(WelcomeNotification.timestamp.desc()).all()
    ...
    for notif in regular_notifs:
        notif.is_read = True
    for notif in welcome_notifs:
        db.session.delete(notif)  # حذف اعلان‌های خوش‌آمدگویی بعد از نمایش
    
    db.session.commit()
    
    unread_count = Notification.query.filter_by(user_id=current_user.id, is_read=False).count()
    
    return render_template_string(NOTIFICATIONS_TEMPLATE, notifications=all_notifs, unread_count=unread_count)
...
<form method="POST" enctype="multipart/form-data">
    <div style="text-align: center; margin-bottom: 20px;">
        <div style="width: 120px; height: 120px; border: 3px dashed #9B5DE5; border-radius: 50%; margin: 0 auto 15px; display: flex; align-items: center; justify-content: center; cursor: pointer;" onclick="document.getElementById('profile_pic').click()" id="upload-area">
            <span style="font-size: 40px;" id="plus-icon">➕</span>
        </div>
...
<div class="filters">
    <a href="{{ url_for('toggle_filter', filter_name='same_age') }}" class="filter-btn {% if filters.get('same_age') %}active{% endif %}">هم‌سن</a>
    <a href="{{ url_for('toggle_filter', filter_name='same_gender') }}" class="filter-btn {% if filters.get('same_gender') %}active{% endif %}">هم‌جنسیت</a>
    <a href="{{ url_for('toggle_filter', filter_name='opposite_gender') }}" class="filter-btn {% if filters.get('opposite_gender') %}active{% endif %}">ناهم‌جنسیت</a>
    <a href="{{ url_for('toggle_filter', filter_name='same_city') }}" class="filter-btn {% if filters.get('same_city') %}active{% endif %}">هم‌شهر</a>
</div>
...
<div class="card">
    <div style="display: flex; align-items: center;">
        ...
        <div style="display: flex; flex-direction: column; gap: 10px;">
            <button onclick="likeUser({{ user.id }})" class="action-btn like-btn {% if liked_users.get(user.id) %}liked{% endif %}" id="like-{{ user.id }}">❤️</button>
