...
// تابع برای مدیریت فیلترها
function handleFilterClick(filterName) {
    const form = document.getElementById('filter-form');
    const oppositeGenderBtn = document.getElementById('opposite-gender-btn');
    const sameGenderBtn = document.getElementById('same-gender-btn');
    
    // اگر فیلتر جنسیت مخالف فعال شود، فیلتر همجنسیت غیرفعال شود و برعکس
    if (filterName === 'opposite_gender' && oppositeGenderBtn.classList.contains('active')) {
        sameGenderBtn.classList.remove('active');
        form.opposite_gender.value = '1';
        form.same_gender.value = '0';
    } else if (filterName === 'same_gender' && sameGenderBtn.classList.contains('active')) {
        oppositeGenderBtn.classList.remove('active');
        form.same_gender.value = '1';
        form.opposite_gender.value = '0';
    }
...
def home():
    ...
    return render_template_string(HOME_TEMPLATE, users=users, filters=filters, unread_count=unread_count)
...
def home_filters():
    ...
    # اگر همجنسیت و ناهمجنسیت همزمان فعال باشند، ناهمجنسیت را غیرفعال کن
    if filters['same_gender'] and filters['opposite_gender']:
        filters['opposite_gender'] = False
...
def delete_chat(partner_id):
    ...
    if chat:
        if for_both:
            ...
            # در اینجا فقط پیام‌ها را حذف می‌کنیم، چت را نگه می‌داریم
...
<body>
    ...
    <main class="scrollable-content">
        <div class="home-page">
            <!-- فیلترها -->
            <form method="post" action="{{ url_for('home_filters') }}" id="filter-form" style="margin-bottom: 20px;">
                <input type="hidden" name="same_age" value="{% if filters.same_age %}1{% else %}0{% endif %}">
                <input type="hidden" name="same_gender" value="{% if filters.same_gender %}1{% else %}0{% endif %}">
                <input type="hidden" name="opposite_gender" value="{% if filters.opposite_gender %}1{% else %}0{% endif %}">
                <input type="hidden" name="same_city" value="{% if filters.same_city %}1{% else %}0{% endif %}">
                
                <div class="filters">
                    <button type="button" onclick="handleFilterClick('same_age')" 
                            class="filter-btn {% if filters.same_age %}active{% endif %}">
                        هم‌سن
                    </button>
                    <button type="button" onclick="handleFilterClick('same_gender')" id="same-gender-btn"
                            class="filter-btn {% if filters.same_gender %}active{% endif %}">
                        هم‌جنسیت
                    </button>
                    <button type="button" onclick="handleFilterClick('opposite_gender')" id="opposite-gender-btn"
                            class="filter-btn {% if filters.opposite_gender %}active{% endif %}">
                        ناهم‌جنسیت
                    </button>
                    <button type="button" onclick="handleFilterClick('same_city')" 
