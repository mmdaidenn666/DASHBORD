...
def logout():
    session.clear()
    if request.referrer and '/teacher' in request.referrer:
        return redirect(url_for('teacher_login'))
    return redirect(url_for('index'))
...
def manage_teachers():
    ...
    return render_template_string(manage_teachers_html, teachers=teachers)
...
@app.route('/manage-reports/<sender_type>')
def manage_reports(sender_type):
    if 'admin_id' not in session:
        return redirect(url_for('admin_login'))
    
    reports = Report.query.filter_by(sender_type=sender_type).all()
    return render_template_string(manage_reports_html, reports=reports, sender_type=sender_type)

@app.route('/manage-lab')
def manage_lab():
    if 'admin_id' not in session:
        return redirect(url_for('admin_login'))
    
    labs = Lab.query.all()
    return render_template_string(manage_lab_html, labs=labs)
...
def add_lab():
    ...
    lab = Lab(images=images, description=description)
    db.session.add(lab)
    db.session.commit()
    return jsonify(success=True)

@app.route('/edit-lab/<int:lab_id>', methods=['POST'])
def edit_lab(lab_id):
    if 'admin_id' not in session:
        return redirect(url_for('admin_login'))
    
    lab = Lab.query.get_or_404(lab_id)
    lab.images = request.form.get('images', '')
    lab.description = request.form.get('description', '')
    db.session.commit()
    return jsonify(success=True)

@app.route('/delete-lab/<int:lab_id>')
...
def manage_grades():
    ...
    return render_template_string(manage_grades_html, students=students)
...
def update_grade(student_id):
    ...
    grade_record = Grade.query.filter_by(student_id=student_id, subject=data['subject']).first()
    ...
    grade_record.midterm1 = data.get('midterm1')
    grade_record.exam1 = data.get('exam1')
    grade_record.midterm2 = data.get('midterm2')
    grade_record.exam2 = data.get('exam2')
    
    db.session.commit()
    return jsonify(success=True)

@app.route('/manage-report-cards')
def manage_report_cards():
    if 'admin_id' not in session:
        return redirect(url_for('admin_login'))
    
    students = Student.query.all()
    return render_template_string(manage_report_cards_html, students=students)

@app.route('/add-report-card', methods=['POST'])
def add_report_card():
    if 'admin_id' not in session:
        return redirect(url_for('admin_login'))
    
    student_id = request.form['student_id']
    # در حالت واقعی، تصویر کارنامه ذخیره می‌شود
    image_path = 'path/to/card.jpg'
    
    card = ReportCard(student_id=student_id, image_path=image_path)
    existing = ReportCard.query.filter_by(student_id=student_id).first()
...
<body>
    ...
    <h1 class="section-title">لیست معلمان</h1>
    ...
    <div class="teachers-container">
        ...
        <div class="teacher-card">
            <div class="teacher-info">
                ...
                <div class="teacher-details">پایه: {{ teacher.grades }}, رشته: {{ teacher.fields }}, درس: {{ teacher.subjects }}</div>
            </div>
    ...
    <script>
        ...
        function deleteTeacher(id) {
            if(confirm('آیا مطمئن هستید می‌خواهید اطلاعات معلم را پاک کنید؟')) {
                fetch('/delete-teacher/' + id)
                .then(response => response.json())
                .then(data => {
                    if(data.success) location.reload();
                });
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>گزارشات {{ sender_type }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { 
            font-family: Tahoma, sans-serif; 
...
<body>
    ...
    <h1 class="section-title">گزارشات</h1>
    
    <div class="reports-container">
        {% for report in reports %}
        <div class="report-card">
            <div class="report-header">
                نام: {{ report.sender_name }}
                {% if report.grade %} - پایه: {{ report.grade }}{% endif %}
                {% if report.field %} - رشته: {{ report.field }}{% endif %}
                {% if report.subject %} - درس: {{ report.subject }}{% endif %}
            </div>
...
<body>
    ...
    <h1 class="section-title">آزمایشگاه</h1>
    ...
    <div class="labs-container">
        ...
        <div class="lab-card">
            <div class="lab-content">
                {% if lab.images %}
                    {% for img in lab.images.split(',') %}
                    <img src="{{ img }}" class="lab-img" alt="عکس آزمایش">
                    {% endfor %}
                {% endif %}
            </div>
            <div class="lab-desc">{{ lab.description or 'توضیحات ندارد' }}</div>
            <button class="edit-btn" onclick="editLab({{ lab.id }})">✎ ویرایش</button>
            <button class="delete-btn" onclick="deleteLab({{ lab.id }})">🗑 حذف</button>
        </div>
        {% endfor %}
    </div>
    ...
    <script>
        ...
        function editLab(id) {
            alert('ویرایش آزمایش ' + id);
        }
        
        function deleteLab(id) {
            if(confirm('آیا مطمئن هستید می‌خواهید این آزمایش را پاک کنید؟')) {
                fetch('/delete-lab/' + id)
                .then(response => response.json())
                .then(data => {
                    if(data.success) location.reload();
...
<body>
    ...
    <h1 class="section-title">لیست دانش آموزان</h1>
    
    <div class="students-container">
        {% for student in students %}
        <div class="student-card" onclick="location.href='/view-student/{{ student.id }}'">
            <div class="student-info">{{ student.first_name }} {{ student.last_name }}</div>
            <div>کد ملی: {{ student.national_code }}</div>
        </div>
        {% endfor %}
    </div>
...
<body>
    ...
    <h1 class="section-title">انتخاب دانش آموز</h1>
