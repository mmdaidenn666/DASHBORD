...
<body>
    ...
    <script>
        ...
        // تابع برای بارگذاری خودکار پیام‌ها
        function loadMessages(chatId) {
            // این قسمت می‌تواند با WebSocket یا polling پیاده‌سازی شود
            setInterval(() => {
                ...
                .then(data => {
                    ...
                    if (data.new_messages && data.new_messages.length > 0) {
                        data.new_messages.forEach(msg => {
                            ...
                            messageDiv.className = msg.sender_id == current_user_id ? 'message me' : 'message them';
...
{% block title %}پروفایل - GOYIMIX{% endblock %}
...
<div class="profile-page">
    ...
    <div style="margin-top: 30px; text-align: center;">
        ...
        <button onclick="if(confirm('آیا مطمئنید؟')) location.href='{{ url_for('delete_account') }}'" 
                class="neon-btn" 
                style="background: linear-gradient(90deg, #ff4757, #ff6b81); margin-top: 10px;">
            حذف حساب
        </button>
    </div>
</div>
...
{% block title %}اعلانات - GOYIMIX{% endblock %}
...
{% block title %}چت‌ها - GOYIMIX{% endblock %}
...
{% block title %}چت با {{ partner.name or partner.username }} - GOYIMIX{% endblock %}
