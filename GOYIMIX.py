...
def home():
    ...
    if 'ناهم‌جنسیت' in filters:
        opposite = 'دختر' if user['gender'] == 'پسر' else 'پسر' if user['gender'] == 'دختر' else 'دیگر'
        query += ' AND gender = ?'
    ...
    return render_template_string(HOME_TEMPLATE, profiles=profiles, filters=filters)
...
def edit_profile():
    ...
    db.execute('''
        UPDATE users SET name=?, age=?, gender=?, bio=?, city=?, password=?, visible=?
        WHERE id=?
    ''', (
...
def like(user_id):
    ...
    if existing:
        ...
        db.execute('INSERT INTO notifications (user_id, message, type, related_id, timestamp) VALUES (?, ?, ?, ?, ?)',
                   (user_id, f"کاربر @{liker['username']} شما را لایک کرد", "like", session['user_id'], datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
    db.commit()
...
def search():
    ...
    db = get_db()
    if query.startswith('@'):
        users = db.execute('SELECT * FROM users WHERE username LIKE ? AND visible = 1', (f"{query[1:]}%",)).fetchall()
    else:
        users = db.execute('SELECT * FROM users WHERE name LIKE ? AND visible = 1', (f"{query}%",)).fetchall()
...
def delete_chat(chat_id):
    ...
    db = get_db()
    chat = db.execute('SELECT * FROM chats WHERE id = ?', (chat_id,)).fetchone()
    if chat and (chat['user1'] == session['user_id'] or chat['user2'] == session['user_id']):
...
def delete_account():
    ...
    password = request.form['password']
    db = get_db()
    user = db.execute('SELECT * FROM users WHERE id = ? AND username = ? AND password = ?', (session['user_id'], username, password)).fetchone()
    if user:
        # حذف اعلان‌ها
        db.execute('DELETE FROM notifications WHERE user_id = ?', (session['user_id'],))
        # حذف لایک‌ها
        db.execute('DELETE FROM likes WHERE liker_id = ? OR liked_id = ?', (session['user_id'], session['user_id']))
        # حذف پیام‌ها و چت‌ها
        chats = db.execute('SELECT id FROM chats WHERE user1 = ? OR user2 = ?', (session['user_id'], session['user_id'])).fetchall()
        for chat in chats:
            db.execute('DELETE FROM messages WHERE chat_id = ?', (chat['id'],))
        db.execute('DELETE FROM chats WHERE user1 = ? OR user2 = ?', (session['user_id'], session['user_id']))
        # حذف کاربر
        db.execute('DELETE FROM users WHERE id = ?', (session['user_id'],))
        db.commit()
        session.pop('user_id', None)
        return redirect(url_for('register'))
    return render_template_string(DELETE_ACCOUNT_TEMPLATE, error="اطلاعات اشتباه است.")
    ...
    <div class="container">
        ...
        <form method="POST">
            <div class="profile-upload">
                <div class="profile-circle plus" onclick="document.getElementById('profile_pic').click()">
...
<body>
    <div class="header">
        ...
        <div class="notification" onclick="window.location.href='/notifications'">
            🔔
    ...
    <div class="filters">
        <button class="filter-btn {% if 'هم‌سن' in filters %}active{% endif %}" onclick="toggleFilter('هم‌سن')">هم‌سن</button>
        <button class="filter-btn {% if 'هم‌جنسیت' in filters %}active{% endif %}" onclick="toggleFilter('هم‌جنسیت')">هم‌جنسیت</button>
        <button class="filter-btn {% if 'ناهم‌جنسیت' in filters %}active{% endif %}" onclick="toggleFilter('ناهم‌جنسیت')">ناهم‌جنسیت</button>
        <button class="filter-btn {% if 'هم‌شهر' in filters %}active{% endif %}" onclick="toggleFilter('هم‌شهر')">هم‌شهر</button>
    </div>
...
<body>
    <div class="header">
        <div class="logo">GOYIMIX | گویمیکس</div>
        <div class="notification" onclick="window.location.href='/notifications'">🔔</div>
    </div>
    ...
    <div class="container">
        ...
        <form method="POST" action="/edit_profile">
            ...
            <div style="margin: 20px 0;">
                ...
                <label class="switch">
                    <input type="checkbox" name="visible" {% if user.visible == 1 %}checked{% endif %}>
        ...
        <a href="/delete_account_page" class="btn danger-btn">حذف حساب</a>
    </div>
    ...
    <div class="container">
        ...
        <div class="chat-card">
            ...
            <div class="chat-info" onclick="window.location.href='/chat/{{ chat.id }}'">
                ...
                <div class="chat-time">{{ chat.last_time or '' }}</div>
            </div>
...
<body>
    <div class="header">
        ...
        <div class="search-container">
            <input type="text" class="search-input" id="searchInput" placeholder="جستجو با @نام_کاربری یا نام" value="{{ request.args.get('q', '') }}">
    ...
    <div class="results-container" id="results">
        ...
        <div class="profile-card">
            ...
            <div class="profile-info">
                ...
                <div class="profile-details">{{ user.age }} سال | {{ user.gender }} | {{ user.city }}</div>
                <div class="profile-details">{{ user.bio or 'بدون بیو' }}</div>
            </div>
        </div>
        {% endfor %}
    </div>
...
<head>
    ...
    <style>
        ...
        .btn:hover {
            transform: translateY(-2px);
            box-shadow: 0 5px 20px rgba(241, 91, 181, 0.7);
        }
        .btn:disabled {
            background: #666;
            cursor: not-allowed;
...
<body>
    ...
    <div class="container">
        <div class="warning">
            <h3>⚠️ هشدار مهم</h3>
            <p>با حذف حساب کاربری خود، تمام اطلاعات شخصی، پیام‌ها، لایک‌ها و ارتباطات شما برای همیشه حذف خواهند شد و امکان بازیابی آن‌ها وجود ندارد.</p>
        </div>
        ...
        <form method="POST" action="/delete_account" id="deleteForm">
            ...
            <div class="checkbox-group">
                ...
                <label for="confirm">آیا مطمئنید که می‌خواهید حساب خود را حذف کنید؟</label>
            </div>
            
            <button type="submit" class="btn" id="deleteBtn" disabled>حذف حساب کاربری</button>
        </form>
    </div>
