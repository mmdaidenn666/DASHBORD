...
def admin_dashboard():
    ...
    <body>
        ...
        <!-- Menu Icon -->
        <div id="menu-icon" onclick="toggleMenu()" style="position: fixed; top: 20px; right: 20px; cursor: pointer; z-index: 1001;">
            <div style="width: 30px; height: 4px; background-color: white; margin: 6px 0; border-radius: 2px;"></div>
            <div style="width: 30px; height: 4px; background-color: white; margin: 6px 0; border-radius: 2px;"></div>
            <div style="width: 30px; height: 4px; background-color: white; margin: 6px 0; border-radius: 2px;"></div>
        </div>

        <!-- Sidebar Menu -->
        <div id="sidebar-menu" style="position: fixed; top: 0; right: -300px; width: 300px; height: 100%; background: rgba(0, 0, 0, 0.9); backdrop-filter: blur(10px); border-left: 2px solid rgba(255, 255, 255, 0.1); display: flex; flex-direction: column; justify-content: center; align-items: center; transition: right 0.5s ease-in-out; z-index: 1000;">
            <a href="/admin-dashboard" class="toolbar-item" style="margin: 10px 0; text-decoration: none; color: white;">
                <span class="toolbar-icon">🏠</span>
                <span>صفحه اصلی</span>
            </a>
            <a href="/announcements" class="toolbar-item" style="margin: 10px 0; text-decoration: none; color: white;">
                <span class="toolbar-icon">📢</span>
                <span>اعلانات</span>
            </a>
            <a href="/profile" class="toolbar-item" style="margin: 10px 0; text-decoration: none; color: white;">
                <span class="toolbar-icon">👤</span>
                <span>پروفایل</span>
            </a>
        </div>
        ...
        <style>
            ...
            * {{
            ...
            .student-card {{
                background: rgba(255, 255, 255, 0.1);
                padding: 20px;
                border-radius: 20px; /* گوشه‌های خمیده */
                border: 1px solid rgba(255, 255, 255, 0.2);
                min-height: 180px;
                display: flex;
                flex-direction: column;
    ...
    <body>
        <div class="container">
            ...
            <div class="students-grid" id="students-list">
                {''.join([
                    f'''
                    <div class="student-card" onclick="showStudentDetails({s['id']})">
                        <div class="student-card-header">
                            <div class="student-name">{s['name']} {s['family']}</div>
                            <div class="student-card-actions">
                                <button class="edit-card-btn" onclick="editStudent(event, {s['id']}); event.stopPropagation();">✎</button>
                                <button class="delete-card-btn" onclick="deleteStudent(event, {s['id']}); event.stopPropagation();">🗑️</button>
                            </div>
                        </div>
        ...
        <button class="add-student-btn" onclick="window.location.href='/add-student/{grade}/{field}'">+</button>
        ...
        <script>
            ...
            function editStudent(event, id) {{
                ...
                window.location.href = `/add-student/{grade}/{field}?id=${{id}}`;
            }}
            ...
            function confirmDelete() {{
                if (studentToDelete) {{
                    fetch('/api/student/' + studentToDelete, {{ method: 'DELETE' }})
...
@app.route("/add-student/<int:grade>/<string:field>", methods=["GET", "POST"])
def add_student_page(grade, field):
    """صفحه افزودن/ویرایش دانش‌آموز."""
    user, role = get_current_user()
    if not user or role != "admin":
        return redirect(url_for("index"))
    ...
    if is_edit:
        data = load_data()
        for s in data.get("students", {}).get(str(grade), {}).get(field, []):
    ...
    if request.method == "POST":
        ...
        if not name or not family or not national_id:
            error = "لطفاً فیلدهای اجباری را پر کنید."
            return render_template_string(add_student_html, error=error, is_edit=is_edit, student=student_data, grade=grade, field=field)
        ...
        for s in students_list:
            if s["id"] != student_id and s["national_id"] == national_id:
        ...
        save_data(data)
        return redirect(url_for("field_page", grade=grade, field=field))

    return render_template_string(add_student_html, is_edit=is_edit, student=student_data, grade=grade, field=field)
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{% if is_edit %}ویرایش{% else %}ثبت{% endif %} دانش‌آموز</title>
    <link href="https://v1.fontapi.ir/css/Vazirmatn" rel="stylesheet">
    <style>
        @import url('https://v1.fontapi.ir/css/Vazirmatn');
        * {
...
<body>
    <button class="back-btn" onclick="window.location.href='/grade/{{ grade }}/{{ field }}'">بازگشت</button>
    <div class="container">
        <div class="header">
            <h1>{% if is_edit %}ویرایش{% else %}ثبت{% endif %} دانش‌آموز</h1>
        </div>
        <div class="form-container">
            <form method="POST">
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="family" value="{{ student.family if student else '' }}" required>
                </div>
                <div class="form-group">
                    <label>کد ملی دانش آموز (اجباری)</label>
                    <input type="text" name="national_id" value="{{ student.national_id if student else '' }}" required>
                </div>
                <div class="form-group">
                    <label>شماره دانش آموز (اختیاری)</label>
                    <input type="text" name="student_number" value="{{ student.student_number if student else '' }}">
                </div>
                <div class="form-group">
                    <label>شماره پدر (اختیاری)</label>
                    <input type="text" name="father_phone" value="{{ student.father_phone if student else '' }}">
                </div>
                <div class="form-group">
                    <label>شماره مادر (اختیاری)</label>
                    <input type="text" name="mother_phone" value="{{ student.mother_phone if student else '' }}">
                </div>
...
def view_student(student_id):
    ...
    for grade_data in data.get("students", {}).values():
        for field_data in grade_data.values():
            for s in field_data: # اصلاح حلقه
                if s.get("id") == student_id:
                    student = s
                    break
            if student:
        ...
        <style>
            ...
            * {{
            ...
            .detail-value {{
                flex-grow: 1;
                padding: 0 20px;
                border: 1px solid rgba(255, 255, 255, 0.1); /* دور هر فیلد خط کشیده شده */
                padding: 5px 10px;
                border-radius: 5px;
            }}
            .back-btn {{
...
def teacher_dashboard():
    ...
    <body>
        ...
        <!-- Sidebar Menu -->
        <div id="sidebar-menu" style="position: fixed; top: 0; right: -300px; width: 300px; height: 100%; background: rgba(0, 0, 0, 0.9); backdrop-filter: blur(10px); border-left: 2px solid rgba(255, 255, 255, 0.1); display: flex; flex-direction: column; justify-content: center; align-items: center; transition: right 0.5s ease-in-out; z-index: 1000;">
            <a href="/teacher-dashboard" class="toolbar-item" style="margin: 10px 0; text-decoration: none; color: white;">
                <span class="toolbar-icon">🏠</span>
                <span>صفحه اصلی</span>
            </a>
            <a href="/announcements" class="toolbar-item" style="margin: 10px 0; text-decoration: none; color: white;">
...
def api_student(student_id=None):
    """API برای افزودن، ویرایش یا حذف دانش‌آموز."""
    user, role = get_current_user()
    if not user or role != "admin":
        return jsonify({"error": "Unauthorized"}), 401
    ...
    if request.method == "POST":
        ...
        # بررسی تکراری بودن کد ملی در صورت تغییر
