...
<body>
    ...
    <div class="navbar">
        ...
        <a href="{{ url_for('notifications') }}" class="nav-item {% if request.endpoint == 'notifications' %}active{% endif %}">
            ...
            {% set unread_count = 0 %}
            {% if current_user.is_authenticated %}
                {% set unread_count = db.session.query(Notification).filter_by(user_id=current_user.id, is_read=False).count() %}
            {% endif %}
            {% if unread_count > 0 %}
...
<form method="POST" enctype="multipart/form-data">
    <div style="text-align: center; margin-bottom: 20px;">
        <div class="upload-area" onclick="document.getElementById('profile_pic').click()" id="upload-area">
