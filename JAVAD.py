...
def delete_student():
    ...
    try:
        ...
        session.modified = True
        # لاگ‌اوت دانش‌آموز و والدین مرتبط
        national_id = student['national_id']
        if 'student' in session and session['student']['national_id'] == national_id:
            student_logout()
        for parent_id in logged_in_parents.copy():
...
def teacher_login():
    ...
    if request.method == 'POST':
        ...
        grades = [request.form.get(f'grade_{i}', '') for i in range(1, num_grades + 1)]
        num_fields = int(request.form.get('num_fields', 0))
        fields = [request.form.get(f'field_{i}', '') for i in range(1, num_fields + 1)]
        num_lessons = int(request.form.get('num_lessons', 0))
        lessons = [request.form.get(f'lesson_{i}', '') for i in range(1, num_lessons + 1)]
        password = request.form.get('password', '')
        unique_id = f"{name}_{family}"
        if not all([name, family, password]) or not grades or not fields or not lessons:
            error = "لطفاً تمامی فیلدها را پر کنید."
    ...
    <div class="form-container">
        ...
        <form method="POST" id="teacherForm">
            ...
            <div class="form-group">
                ...
                <select name="num_grades" id="numGrades" required onchange="updateGrades()">
                    <option value="">انتخاب کنید...</option>
            ...
            <div class="form-group">
                ...
                <select name="num_fields" id="numFields" required onchange="updateFields()">
                    <option value="">انتخاب کنید...</option>
                    <option value="1">1</option>
                    <option value="2">2</option>
                    <option value="3">3</option>
            ...
            <div class="form-group">
                ...
                <select name="num_lessons" id="numLessons" required onchange="updateLessons()">
                    <option value="">انتخاب کنید...</option>
                    <option value="1">1</option>
                    <option value="2">2</option>
                    <option value="3">3</option>
        ...
        function updateLessons() {
            ...
            for (let i = 1; i <= num; i++) {
                container.innerHTML += `
                    <div class="form-group">
                        <label>درس ${i}</label>
                        <input type="text" name="lesson_${i}" required>
                    </div>
                `;
            }
        }
...
def teacher_profile():
    ...
    if request.method == 'POST':
        ...
        value = data.get('value')
        if field == 'num_grades':
            session['teacher']['grades'] = ['' for _ in range(int(value))]
        elif field == 'num_fields':
            session['teacher']['fields'] = ['' for _ in range(int(value))]
        elif field == 'num_lessons':
            session['teacher']['lessons'] = ['' for _ in range(int(value))]
        elif field.startswith('grade_') or field.startswith('field_') or field.startswith('lesson_'):
            index = int(field.split('_')[1]) - 1
            if 'grades' in field:
                session['teacher']['grades'][index] = value
            elif 'fields' in field:
                session['teacher']['fields'][index] = value
            elif 'lessons' in field:
                session['teacher']['lessons'][index] = value
        session.modified = True
        return jsonify(success=True)
...
<body>
    ...
    <div class="container">
        ...
        <div class="profile-field">
            <div class="field-label">تعداد پایه:</div>
            <div class="field-value" id="numGradesValue">{{ user.grades|length }}</div>
            <button class="edit-btn" onclick="editNum('num_grades', this)"><i class="fas fa-pencil-alt"></i></button>
        </div>
        
        {% for i in range(user.grades|length) %}
        <div class="profile-field">
            <div class="field-label">پایه {{ i+1 }}:</div>
            <div class="field-value" id="grade_{{ i+1 }}Value">{{ user.grades[i] }}</div>
            <button class="edit-btn" onclick="editField('grade_{{ i+1 }}', this)"><i class="fas fa-pencil-alt"></i></button>
        </div>
        {% endfor %}
        
        <div class="profile-field">
            <div class="field-label">تعداد رشته:</div>
            <div class="field-value" id="numFieldsValue">{{ user.fields|length }}</div>
            <button class="edit-btn" onclick="editNum('num_fields', this)"><i class="fas fa-pencil-alt"></i></button>
        </div>
        
        {% for i in range(user.fields|length) %}
        <div class="profile-field">
            <div class="field-label">رشته {{ i+1 }}:</div>
            <div class="field-value" id="field_{{ i+1 }}Value">{{ user.fields[i] }}</div>
            <button class="edit-btn" onclick="editField('field_{{ i+1 }}', this)"><i class="fas fa-pencil-alt"></i></button>
        </div>
        {% endfor %}
        
        <div class="profile-field">
            <div class="field-label">تعداد درس:</div>
            <div class="field-value" id="numLessonsValue">{{ user.lessons|length }}</div>
            <button class="edit-btn" onclick="editNum('num_lessons', this)"><i class="fas fa-pencil-alt"></i></button>
        </div>
        
        {% for i in range(user.lessons|length) %}
        <div class="profile-field">
            <div class="field-label">درس {{ i+1 }}:</div>
            <div class="field-value" id="lesson_{{ i+1 }}Value">{{ user.lessons[i] }}</div>
            <button class="edit-btn" onclick="editField('lesson_{{ i+1 }}', this)"><i class="fas fa-pencil-alt"></i></button>
        </div>
        {% endfor %}
    ...
    <script>
        ...
        function editField(field, btn) {
            ...
            if (field.startsWith('grade_') || field.startsWith('field_')) {
                const select = document.createElement('select');
                select.style.cssText = 'width: 100%; padding: 10px; background: #222; color: white; border-radius: 10px; border: 1px solid #444; font-size: 18px;';
        ...
        function saveEdit(field, btn) {
            ...
            if (field.startsWith('num_')) {
                const select = valueEl.querySelector('select');
                newValue = select ? select.value : '';
            } else if (field.startsWith('grade_') || field.startsWith('field_')) {
                const select = valueEl.querySelector('select');
                newValue = select ? select.value : '';
            } else {
                const input = valueEl.querySelector('input');
...
def teacher_attendance():
    ...
    if request.method == 'POST':
        ...
        absent = [s['national_id'] for s in get_students_by_grade_field(grade, field) if s['national_id'] not in present]
        
        # Format message
        teacher_name = f"{teacher['name']} {teacher['family']}"
        lessons_str = ', '.join(teacher['lessons'])
        msg = f"""
        <div style="text-align: center; font-weight: bold;">{teacher_name} - {grade} - {field} - {lessons_str}</div>
        <div>تاریخ: {date}</div>
        <div style="display: flex; justify-content: space-between;">
            <div>
                <span style="color: red;">غائبین</span><br>
                { '<br>'.join([find_student(grade, field, nid)['name'] + ' ' + find_student(grade, field, nid)['family'] for nid in absent]) or 'هیچ' }
            </div>
            <div>
                <span style="color: green;">حاضرین</span><br>
                { '<br>'.join([find_student(grade, field, nid)['name'] + ' ' + find_student(grade, field, nid)['family'] for nid in present]) or 'هیچ' }
            </div>
        </div>
        """
        ...
        notify_admin(msg)
        notify_teacher(unique_id, msg)
        
        for nid in present:
            notify_student(nid, f"شما در کلاس {teacher_name} - {grade} - {field} - {lessons_str} در تاریخ {date} حضور داشته‌اید.")
        for nid in absent:
            notify_student(nid, f"شما در کلاس {teacher_name} - {grade} - {field} - {lessons_str} در تاریخ {date} حضور نداشته‌اید.")
        
        # Notify parents
        for parent_id in logged_in_parents:
            parent = session.get('parent')  # Note: this assumes session is shared, but in practice, need to store parent-child mapping better
            if parent and parent['grade'] == grade and parent['field'] == field:
                child1_nid = parent['child1'].split('|')[2]
                child2_nid = parent.get('child2', '').split('|')[2] if parent.get('child2') else None
                if child1_nid in present:
                    notify_parent(parent_id, f"{parent['child1'].split('|')[0]} {parent['child1'].split('|')[1]} در کلاس {teacher_name} - {grade} - {field} - {lessons_str} در تاریخ {date} حضور داشته است.")
                elif child1_nid in absent:
                    notify_parent(parent_id, f"{parent['child1'].split('|')[0]} {parent['child1'].split('|')[1]} در کلاس {teacher_name} - {grade} - {field} - {lessons_str} در تاریخ {date} حضور نداشته است.")
                if child2_nid and child2_nid in present:
                    notify_parent(parent_id, f"{parent['child2'].split('|')[0]} {parent['child2'].split('|')[1]} در کلاس {teacher_name} - {grade} - {field} - {lessons_str} در تاریخ {date} حضور داشته است.")
                elif child2_nid and child2_nid in absent:
                    notify_parent(parent_id, f"{parent['child2'].split('|')[0]} {parent['child2'].split('|')[1]} در کلاس {teacher_name} - {grade} - {field} - {lessons_str} در تاریخ {date} حضور نداشته است.")
...
<head>
    ...
    <style>
        ...
        button { width: 100%; padding: 10px; background: #FF3CAC; color: black; border: none; }
...
<body>
    ...
    <form method="POST">
        <div class="form-group">
            <label>پایه</label>
            <select name="grade" id="gradeSelect" required onchange="loadStudents()">
                <option value="">انتخاب کنید...</option>
        ...
        <div class="form-group">
            <label>رشته</label>
            <select name="field" id="fieldSelect" required onchange="loadStudents()">
                <option value="">انتخاب کنید...</option>
    ...
    <script>
        async function loadStudents() {
            ...
            if (grade && field) {
                const response = await fetch(`/teacher/get_students?grade=${grade}&field=${field}`);
                ...
                students.forEach(student => {
                    list.innerHTML += `
                        <div class="student-item">
                            <label>
                                <input type="checkbox" name="present" value="${student.national_id}" checked>
...
def parent_login():
    ...
    if request.method == 'POST':
        ...
        if not all([name, family, grade, field, child1, password]):
            error = "لطفاً تمامی فیلدها را پر کنید. فرزند اول اجباری است."
...
<body>
    <div class="form-container">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <select name="grade" required onchange="updateFields()">
                    <option value="">انتخاب کنید...</option>
    ...
    <script>
        ...
        function updateStudents() {
            ...
            if (grade && field) {
                // Fetch students via AJAX if needed, but since static, use students var
                students.forEach(student => {
    ...
    <script>
        ...
        function editField(field, btn) {
            ...
            if (field === 'grade') {
                ...
                // Would fetch students based on current grade/field, but for simplicity assume reload or AJAX
                // Here, placeholder
                valueEl.appendChild(select);
            } else {
                const input = document.createElement('input');
